<template>
  <div>
    <h2 class="style-title">Devices groups</h2>
    <vs-button color="primary" type="filled" to="/devices_groups/add" style="margin-bottom:20px;">Add device group</vs-button>
    <vs-table data="users">
...
export default {
    ...
    methods:{
      getData(){
        axios.get('/api/admin/devices_groups').then((res) =>{
          console.log(res);
        }).catch((err) => {
          console.log(err);
        })
      ...
      deleteDeviceGroup(){
        axios.delete('/api/admin/tes3/' + this.id).then((res) =>{
          console.log(res);
        }).catch((err) => {
          console.log(err);
        })
