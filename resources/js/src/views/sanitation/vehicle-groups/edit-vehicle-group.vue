<template>
	<div class="max-width-500">
		<vs-alert color="danger" v-if="data.name.length < 3" style="margin-bottom:20px;">
  		Vehicle group name must me longer that three letters.
		</vs-alert>
		<vs-alert color="success" v-if="alert" style="margin-bottom:20px;">
  		{{alert}}
		</vs-alert>
		<h2 class="style-title">Edit vehicle group</h2>
...
export default {
    ...
    methods: {
        getData(){
            ...
            this.$axios
                .get(
                    '/api/admin/vehicle_group/' + this.$route.params.id
        ...
        postRequest(){
            let user = JSON.parse(localStorage.user)
            let token = user.api_token
            let url = '/api/admin/vehicle_group/update/' + this.$route.params.id
            this.$axios({
                method: 'PUT',
                url: url,
                data: this.data,
